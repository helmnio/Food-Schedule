(() => {
  ...
  function setDishTab(tab){
    activeDishTab=tab;
    document.querySelectorAll('[data-dish-tab]').forEach(btn=>btn.classList.toggle('active',btn.dataset.dishTab===tab));
    document.querySelectorAll('[data-dish-panel]').forEach(panel=>panel.hidden=panel.dataset.dishPanel!==tab);
    if(tab==='find') setTimeout(()=>dishSearch?.focus(),0);
  }
  ...
  window.renderDishes=function(){
    ...
    renderSection(document.querySelector('#personalDishGrid'),personalDishes(),'Your created or customised dishes will appear here.');
