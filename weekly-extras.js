(() => {
  ...
  function renderResults(){const q=weeklyItemSearch.value.trim().toLowerCase();if(!q){weeklyItemSuggestions.innerHTML='';return}const used=existing(),counts=usage(),known=unique([...allIngredients(),...(state.previousItems||[]),...Object.keys(counts)]);const matches=known.filter(name=>!used.has(name.toLowerCase())&&name.toLowerCase().includes(q)).slice(0,20);const clean=cleanName(weeklyItemSearch.value);weeklyItemSuggestions.innerHTML=[...matches.map(name=>`<button type="button" class="dish-result" data-weekly-pick="${esc(name)}"><span>${esc(name)}</span></button>`),...(clean?[`<button type="button" class="dish-result custom-add-result" data-weekly-pick="${esc(clean)}"><span>+ Add “${esc(clean)}”</span><small>Custom</small></button>`]:[])].join('')||'<p class="muted ingredient-empty">No matching items.</p>'}
  function close(){weeklyItemDialog.close();weeklyItemSearch.value='';weeklyItemSuggestions.innerHTML=''}
  function add(raw){const name=cleanName(raw||'');if(!name)return;record(name);if(!existing().has(name.toLowerCase()))state.extras.push({name,type:''});save();render();close();toast('Weekly item added')}
  ...
  state.extras=(state.extras||[]).map(item=>typeof item==='string'?{name:item,type:''}:{...item,type:''});
  render();
})();