(() => {
  ...
  function renderResults(){const q=weeklyItemSearch.value.trim().toLowerCase();if(!q){weeklyItemSuggestions.innerHTML='';return}const used=existing(),counts=usage(),known=unique([...allIngredients(),...(state.previousItems||[]),...Object.keys(counts)]);const matches=known.filter(name=>!used.has(name.toLowerCase())&&name.toLowerCase().includes(q));const clean=cleanName(weeklyItemSearch.value);weeklyItemSuggestions.innerHTML=[...matches.map(name=>`<button type="button" class="dish-result" data-weekly-pick="${esc(name)}"><span>${esc(name)}</span></button>`),...(clean?[`<button type="button" class="dish-result custom-add-result" data-weekly-pick="${esc(clean)}"><span>+ Add custom ingredient “${esc(clean)}”</span></button>`]:[])].join('')}
